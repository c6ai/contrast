...
func (a *Authority) SetManifest(ctx context.Context, req *userapi.SetManifestRequest) (*userapi.SetManifestResponse, error) {
	...
	if err := a.checkSeedEngine(); errors.Is(err, ErrNeedsRecovery) {
		return nil, status.Error(codes.FailedPrecondition, err.Error())
	} else if err != nil {
		return nil, status.Errorf(codes.Internal, "checking internal state: %v", err)
	}
	...
	if err := json.Unmarshal(req.GetManifest(), &m); err != nil {
	...
	if oldState == nil {
		...
		seedShares, err := crypto.EncryptSeedShares(seed, m.SeedshareOwnerPubKeys)
		if err != nil {
			return nil, status.Errorf(codes.InvalidArgument, "initializing seed engine: %v", err)
		}
		if err := a.recover(seed, salt); err != nil {
	...
	meshKey, err := a.se.Load().DeriveMeshCAKey(nextTransitionHash)
	if err != nil {
		return nil, status.Errorf(codes.Internal, "deriving mesh CA key: %v", err)
	}
	ca, err := ca.New(a.se.Load().RootCAKey(), meshKey)
	if err != nil {
