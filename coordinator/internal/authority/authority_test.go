...
// TODO(burgerdev): reintroduce tests below.

// func TestSetManifest(t *testing.T) {
// Test that SetManifest can be called twice, and that the gauge is set correctly.

// func TestSetManifest_TooFewPolicies(t *testing.T) {
// tested in userapi_test

// func TestSetManifest_BadManifest(t *testing.T) {
// Test that a broken manifest is not accepted (e.g. []byte(`{ "policies": 1 }`) )

// func TestGetManifestsAndLatestCA(t *testing.T) {
// Test that setting a second manifest works.
// Test that a recovered Authority can read from history and produces the same state.
